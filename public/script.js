...
async function confirmSeatSelection() {
    ...
    try {
        ...
        console.log('Making booking with:', {
            ...
            selectedBusType: typeof selectedBus?.id
        });
        ...
        setTimeout(() => {
            window.location.href = `confirmation.html?bookingId=${currentBooking.id}`;
        }, 2000);
...
function showError(message) {
    // Completely disabled - no error messages
    console.log('Error (hidden):', message);
}

function showSuccess(message) {
    // Only show critical success messages with simple alert
    if (message.includes('Booking successful') || message.includes('Registration successful') || message.includes('Login successful')) {
        alert(message);
    } else {
        console.log('Success (hidden):', message);
    }
}
